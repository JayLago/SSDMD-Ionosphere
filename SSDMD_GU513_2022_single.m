...
model = ssdmd(train_data, wave_levels, wave_type, dmd_tol, ...
    corr_tol, day, dt, num_cols);

% Save forecasted parameters for this period
[fof2, hmf2_idx] = max(model.full_recon, [], 1);
fof2_ssdmd = fof2(test_start_ix:test_stop_ix);
hmf2_ssdmd = cast(heights(hmf2_idx(test_start_ix:test_stop_ix)), 'double');
...
[fof2_, hmf2_] = max(model.full_recon, [], 1);
